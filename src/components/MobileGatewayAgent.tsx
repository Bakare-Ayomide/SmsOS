...
export default function MobileGatewayAgent() {
  ...
  const startQrScanner = async () => {
    ...
    setIsScanning(true);
    addLog("Initiating native camera stream for QR scan...", "info");
    ...
    setTimeout(async () => {
      try {
        ...
        await qrScanner.start(
          ...
          (errorMessage) => {
            // Frame analysis logs can be safely ignored
          }
        );
      } catch (err: any) {
        console.error("Camera scanner start failed:", err);
        setScannerError(err.message || "Failed to access camera. Please check camera permissions.");
        setIsScanning(false);
        addLog(`Camera access denied or failed: ${err.message || String(err)}`, "error");
      }
    }, 200);
  };
  ...
  const handleQrScanSuccess = async (decodedText: string) => {
    ...
    try {
      ...
      if (config.serverUrl && config.gatewayId && config.gatewayToken) {
        setServerUrl(config.serverUrl);
        setGatewayId(config.gatewayId);
        setGatewayToken(config.gatewayToken);
        
        // Auto-save & link right away!
        localStorage.setItem("sms_gw_server_url", config.serverUrl.trim());
        localStorage.setItem("sms_gw_id", config.gatewayId.trim());
        localStorage.setItem("sms_gw_token", config.gatewayToken.trim());
        setIsCoupled(true);
        setShowSettings(false);
        addLog(`✓ Coupled successfully via QR scan credentials!`, "success");
        addLog(`Host SaaS Node: ${config.serverUrl}`, "info");
        addLog(`Device Hooked ID: ${config.gatewayId}`, "info");
      } else {
        throw new Error("Invalid QR code schema. Missing serverUrl, gatewayId, or gatewayToken.");
      }
    } catch (err: any) {
      addLog(`❌ Failed to parse QR config: ${err.message || String(err)}`, "error");
      alert(`Invalid QR Code content: ${err.message || "Ensure you are scanning the QR code generated by the SaaS website."}`);
    }
    stopQrScanner();
  };
          ...
          <div className="bg-slate-900 border border-slate-800/80 rounded-2xl p-5 shadow-xl flex flex-col gap-4">
            ...
            {!isScanning ? (
              <button
                ...
                className="w-full py-3 bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-500 hover:to-violet-500 text-white font-extrabold text-xs rounded-xl flex items-center justify-center gap-2 shadow-lg shadow-indigo-600/15 border border-indigo-500/20 transition-all cursor-pointer active:scale-98"
              >
                <QrCode className="w-4 h-4" />
                <span>Scan Coupling QR Code</span>
              </button>
            ) : (
              <div className="bg-slate-950 border border-slate-800 rounded-2xl p-4 flex flex-col gap-3 relative overflow-hidden">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-bold text-slate-300 flex items-center gap-1.5 font-mono uppercase">
                    <span className="relative flex h-2 w-2">
                      <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-indigo-400 opacity-75"></span>
                      <span className="relative inline-flex rounded-full h-2 w-2 bg-indigo-500"></span>
                    </span>
                    Align QR Code in camera view
                  </span>
                  <button
                    type="button"
                ...
                {/* Viewport for html5-qrcode library */}
                <div 
                  id="qr-reader-viewport" 
                  className="w-full aspect-square bg-slate-900 border border-slate-850 rounded-xl overflow-hidden relative flex items-center justify-center text-slate-500 text-xs text-center p-4 font-mono"
                >
                  {/* Camera guides */}
                  <div className="absolute inset-4 border border-indigo-500/30 rounded-lg pointer-events-none flex items-center justify-center z-10">
                    <div className="w-2/3 h-2/3 border-2 border-indigo-500 border-dashed rounded-md opacity-60"></div>
                  </div>
                  Camera Viewport Active...
                </div>

                {scannerError && (
                  <div className="text-[10px] text-rose-400 bg-rose-500/10 border border-rose-500/20 p-2.5 rounded-xl font-medium leading-relaxed">
                    {scannerError}
                  </div>
                )}
              </div>
            ...
            <div className="flex items-center gap-3 py-1">
              ...
              <span className="text-[9px] font-bold text-slate-500 uppercase tracking-widest font-mono">Or Configure Manually</span>
              <div className="h-[1px] bg-slate-800/80 flex-1"></div>
            </div>
